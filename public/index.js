...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function init() {
        ...
        toggleReadOnlyBtn.addEventListener('click', () => {
            ...
            toggleReadOnlyBtn.title = isReadonly ? 'Activar modo edición' : 'Activar modo solo lectura';
        });

        // --- AI Q&A Modal ---
        askAiBtn.addEventListener('click', () => {
            aiResponseArea.innerHTML = '<p>Escribe tu pregunta a continuación...</p>';
        ...
        async function handleSendAiQuestion() {
            ...
            // Collect all notes context
            let context = '';
            document.querySelectorAll('tr[data-topic-id]').forEach(row => {
                const title = row.querySelector('.topic-text')?.textContent || 'Tema sin título';
                const greyNote = row.dataset.greyNote;
                const blueNote = row.dataset.blueNote;
                let topicContext = '';
                if (greyNote) {
                    topicContext += `\n**Esquema para ${title}:**\n${formatAsText(greyNote)}\n`;
                }
                if (blueNote) {
                    topicContext += `\n**Desarrollo para ${title}:**\n${formatAsText(blueNote)}\n`;
                }
                if (topicContext) {
                     context += `--- TEMA: ${title} ---\n${topicContext}\n`;
                }
            ...
            document.querySelectorAll('tr[data-section-header]').forEach(row => {
                const sectionNote = row.dataset.sectionNote;
                if (sectionNote) {
                    const title = row.querySelector('.section-title')?.textContent || 'Sección sin título';
                    context += `--- NOTA DE SECCIÓN: ${title} ---\n${formatAsText(sectionNote)}\n\n`;
                }
            });
            ...
            try {
                ...
                aiResponseArea.innerHTML = `<p class="text-red-500"><strong>Hubo un error al contactar a la IA:</strong> ${error.message}</p>`;
            } finally {
        ...
        sendAiQaBtn.addEventListener('click', handleSendAiQuestion);
        ...
        // --- Import / Export ---
        const getFileHandle = async (forSave = false) => {
            if ('showOpenFilePicker' in window && !forSave) {
                const [handle] = await window.showOpenFilePicker({
                    types: [{ description: 'JSON Files', accept: { 'application/json': ['.json'] } }]
                });
                return handle;
            }
            if ('showSaveFilePicker' in window && forSave) {
        ...
        importFileInput.addEventListener('change', async (event) => {
            ...
            if (confirm("¿Estás seguro de que quieres importar este archivo? Se sobrescribirá todo tu progreso actual.")) {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if(state.version && parseFloat(state.version) >= 2.0) {
                             loadState(state);
                             saveState(); // Save imported state to localStorage
                             alert('Progreso importado exitosamente!');
                        } else {
                            alert('El archivo de importación es de una versión antigua y no es compatible.');
                        }
                    } catch (err) {
        ...
        [linkModal, notesModal, aiQaModal].forEach(modal => {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === "Escape") {
                // Close the topmost/active element
                if (!settingsDropdown.classList.contains('hidden')) {
                    settingsDropdown.classList.add('hidden');
                } else if (linkModal.classList.contains('visible')) {
                    hideModal(linkModal);
                } else if (aiQaModal.classList.contains('visible')) {
                    hideModal(aiQaModal);
                } else if (notesModal.classList.contains('visible')) {
                    closeNoteModal();
                }
            }
        });
        
        // Auto-detect dark mode changes for the default theme
        window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', e => {
            if (document.documentElement.dataset.theme === 'default') {
