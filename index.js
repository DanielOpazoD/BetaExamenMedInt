...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupEditorToolbar() {
        ...
        const createSymbolDropdown = (symbols, title, icon) => {
            ...
            const content = document.createElement('div');
            content.className = 'symbol-dropdown-content';
            const hasCharManager = title === 'Caracteres Especiales';
            let editContainer;
            if (hasCharManager) {
                editContainer = document.createElement('div');
                editContainer.style.gridColumn = '1 / -1';
                editContainer.className = 'text-right mb-2';
                const editBtn = document.createElement('button');
                editBtn.textContent = 'Editar';
                editBtn.className = 'px-2 py-1 text-xs bg-gray-200 rounded hover:bg-gray-300';
            ...
            const renderSymbols = () => {
                content.innerHTML = '';
                if (hasCharManager && editContainer) content.appendChild(editContainer);
                symbols.forEach((sym) => {
