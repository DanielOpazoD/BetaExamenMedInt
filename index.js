...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupEditorToolbar() {
        ...
        const clinicalBtn = createButton('Importancia clínica', '🩺', null, null, async () => {
            const selection = window.getSelection();
            if (selection.rangeCount > 0 && notesEditor.contains(selection.anchorNode)) {
                savedEditorSelection = selection.getRangeAt(0).cloneRange();
            } else {
            ...
            await generateClinicalImportance();
        });
        editorToolbar.appendChild(clinicalBtn);
        editorToolbar.appendChild(createSeparator());
    ...
    async function generateClinicalImportance() {
        ...
        try {
            ...
            showAlert('Error al generar resumen: ' + error.message);
        } finally {
            notesEditor.focus();
        }
    }
