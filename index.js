...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupEditorToolbar() {
        ...
        const aiSummaryBtn = createButton('Importancia clínica de la enfermedad', '🤖', null, null, insertClinicalImportanceSummary);
        editorToolbar.appendChild(aiSummaryBtn);
        editorToolbar.appendChild(createSeparator());
    ...
    async function insertClinicalImportanceSummary() {
        if (!API_KEY) {
            await showAlert("La API Key de Gemini no está configurada.");
            return;
        }
        ...
        const prompt = `Analiza la nota que el usuario está editando.\nIdentifica la enfermedad principal desde el título.\nRedacta un resumen introductorio que responda brevemente: por qué es importante el problema clínico, cómo afecta a los individuos, a qué complicaciones y patologías se asocia y por qué es importante tratarlo.\nGenera una lista de 4 a 8 viñetas telegráficas (1–2 líneas) en HTML sin contenedor <ul>, cada una con el formato: <li><b>Subtítulo:</b> texto breve</li>.\nLa última viñeta debe ser siempre "Tratamiento: ámbitos principales".\nEl contenido debe basarse primero en el documento actual; si faltan datos, complementar con conocimiento médico.\nDevuelve únicamente las viñetas sin incluir <ul> ni título.\n\nTítulo: ${title}\nContenido:\n${content}`;
        try {
            ...
            const chip = `<!-- Amarillo Claro --><div style="background: linear-gradient(to right, #fffde7, #fff176); color:#795548; font-weight:bold; padding:6px 12px; border-radius:20px; display:inline-block; margin:6px 4px;">Importancia clínica de ${disease}</div>`;
            const summaryHtml = `${chip}<ul style="list-style:none;padding-left:0;margin:6px 0;">${bulletItems}</ul>`;
    ...
    function setupEventListeners() {
        ...
        sendAiQaBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const ai = new GoogleGenAI({ apiKey: API_KEY });
                const fullPrompt = `Basado en las siguientes notas de estudio, responde la pregunta del usuario. Proporciona primero "Razonamiento:" y luego "Respuesta:". Contenido:\n\n${notesContext}\n\nPregunta: ${question}`;
